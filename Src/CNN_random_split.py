...
Die Zellbilder werden zufällig und klassenstratifiziert in Trainings-,
Validierungs- und Testdaten aufgeteilt. Dadurch können Zellen desselben Slides
in mehreren Datensätzen vorkommen. Das Modell wird mit gewichteter binärer
Kreuzentropie trainiert.
...
MODEL_PATH = RESULT_DIR / "best_resnet18_random_split.pth"
...
# Zunächst werden 15 % aller einzelnen Zellbilder als Testdaten abgetrennt.
# Die Klassenverteilung wird durch die stratifizierte Aufteilung beibehalten.
...
# Weitere 15 % des Gesamtdatensatzes werden für die Validierung verwendet.
...
plt.savefig(
    RESULT_DIR / "loss_curve_random_split.png",
    dpi=300,
)
...
plt.savefig(
    RESULT_DIR / "accuracy_curve_random_split.png",
    dpi=300,
)
...
# Die Testdaten werden ohne weitere Gewichtsaktualisierung ausgewertet.
with torch.no_grad():
...
print(
    "Negative:",
    np.sum(np.array(test_targets) == 0)
)
print(
    "Positive:",
    np.sum(np.array(test_targets) == 1)
)
